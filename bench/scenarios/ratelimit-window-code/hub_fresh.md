...
anchors:
  - claim: >
      ...
      return False. So a limiter built with limit=N lets N+1 requests through per window and
      rejects the (N+2)th.
...
The contract callers rely on: a limiter constructed with `limit = N` admits **`N + 1`** requests
in any single window (the bound is off-by-one against the naive reading), and the `(N + 2)`-th
request in that window is throttled.
