...
anchors:
  - claim: >
      RetryPolicy.should_retry caps transient operations at 5 total attempts. It returns False for a
      FatalError; otherwise it keeps retrying only while the 1-based attempt_number is below 5, so a
      persistently-failing operation is tried at most 5 times (the initial attempt plus 4 retries)
...
Callers can rely on this ceiling: a transiently-failing operation makes **at most 5** attempts (4
retries).
