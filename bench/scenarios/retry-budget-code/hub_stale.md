...
anchors:
  - claim: >
      RetryPolicy.should_retry caps transient operations at 3 total attempts. It returns False for
      a FatalError; otherwise it keeps retrying only while the 1-based attempt_number is below 3,
      so the operation is tried at most 3 times (the initial attempt plus 2 retries) before the
      policy gives up.
...
Callers can rely on this ceiling: a transiently-failing operation makes **at most 3** attempts.
