...
> **Findings (authored).** The sections below are auto-generated by `surface_bench.report`; this
> summary is the human read of them. (N=10/cell, haiku/sonnet/opus, single-shot, synthetic curated
> scenarios — see `PROVENANCE.md`.)

## Findings

Rot does two different kinds of damage depending on whether the agent can see the code it depends
on, and the **cascade** family vs the **comprehension** family isolate the two.

**1. When the dependency is hidden, a stale doc breaks every model — and a smarter model doesn't
help.** In the 4 cascade scenarios the agent edits a visible function but the drifted dependency is
withheld (it knows it only by doc). A stale doc (C1) produced **0% success and 100% "misled"** on
**all three models**; a fresh doc (C2) produced **100%**. So H1 (C2−C1) is **+100pp flat across
haiku → sonnet → opus** — capability buys *no* resistance to rot when the truth isn't visible. This
is the headline: you cannot out-model a stale doc about code you can't see.

**2. Surfacing the drift recovers the loss.** Handing the agent the real `surf check` report (C3)
— whose payload includes the corrected code — recovered success to **100% on sonnet and opus** and
**90% on haiku**. That is the Surface value proposition demonstrated end to end (H3).

**3. When the code is visible, rot doesn't break correctness — it taxes tokens.** The 7
comprehension scenarios put the drifted code and its contradicting doc side by side; every model
just reads the code, so success ceilings near 100% regardless of the doc. But a stale doc still
cost **+57 to +107 extra output tokens** vs a fresh one on every model (C1−C2) — the quiet,
universal cost of rot you *can* see.

**4. The shape of the cost differs by family.** In the cascade family a confident stale doc makes
the model **cheaply wrong** — C1 actually spends *fewer* output tokens than C0 (no doc), because
with no doc the model deliberates about the unseen dependency, while a stale doc lets it commit to
the wrong answer immediately; the expense then reappears in **recovery** (C3 is the costliest
condition). In the comprehension family the cost is the steady token tax of reconciling prose
against visible code.

**Caveats.** N=10 per cell; three Claude models; single-shot prompting (no tool-use loop, which
would likely amplify the token-waste signal); synthetic curated scenarios with deterministic
graders. Directional and clean, but a pilot — not a population estimate.
